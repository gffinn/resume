...
function validateTimelineData(data) {
    ...
    data.forEach((item, idx) => {
        ...
        const required = ['id', 'company', 'role', 'dates', 'stack', 'summary', 'details']
        required.forEach((k) => {
        ...
        Object.keys(item).forEach((k) => {
            ...
            if (v && typeof v === 'object' && ('$$typeof' in v || v._store)) {
                console.warn(`timelineData[${idx}].${k} looks like a React element — move it out of data.`)
            }
        })
    })
}
...
export default function Timeline() {
    ...
    if (error) {
        return (
            <div className={styles.container}>
                <h1 className={styles.heading}>Career Timeline</h1>
                <p style={{ color: 'salmon', textAlign: 'center' }}>Failed to load timeline data.</p>
            </div>
        )
    }
                    ...
                    >
                        ...
                        <div className={styles.card}>
                            <div className={styles.cardHeader}>
                                ...
                                >
                                    <ChevronDown
                                        className={`${styles.chevron} ${expanded === item.id ? styles.chevronOpen : ""
                                            }`}
                                    />
                                </button>
                            </div>
